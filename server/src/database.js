...
function createTables() {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS squad_config (id TEXT PRIMARY KEY, auto_interval TEXT DEFAULT '0 0 */2 * *', max_posts_per_cycle INTEGER DEFAULT 3, created_at INTEGER DEFAULT (strftime('%s','now')), updated_at INTEGER DEFAULT (strftime('%s','now')))`);
  ...
  try { db.run('ALTER TABLE leads ADD COLUMN purchased INTEGER DEFAULT 0'); } catch (e) {}
}
